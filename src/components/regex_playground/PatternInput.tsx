...
const FlagToggle = React.memo(function FlagToggle({ k, active, onToggle }: FlagToggleProps) {
    const label = '/' + k;
                ...
                <div className="relative">
                    ...
                    {showLoading && (
                        <div className="absolute top-2 right-2">
                            <div
                                className="loading loading-spinner loading-xs opacity-60"
                                aria-label="Parsing pattern"
                            ></div>
                        </div>
                    )}
                </div>
                <div className="text-xs text-base-content/60 mt-1">
                    Hover over flags for details. Most patterns just need{' '}
                    <span className="font-medium">g</span> (global) enabled.
                </div>
            </div>
        </div>
    );
